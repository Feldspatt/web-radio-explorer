...
export const paths = {
	...
	setServer: (ip: string) => (radioServerUrl = ip + "/json"),
	...
	getStationSearch: (params: URLSearchParams) =>
		radioServerUrl + `/stations/search?${params.toString()}`,
	getByUUID: (uuidArray: string[]) =>
		radioServerUrl + `/stations/byuuid?uuids=${uuidArray.join(",")}`,
};
