...
const App: React.FC = () => {
	...
	useEffect(() => {
		...
		const selectLastListenedStation = async () => {
			...
			const station = (
				await fetchStationsByUUIDs([lastListenedList[0]])
			)[0];
	...
	const handleFilterUpdate = useCallback(
		(filter: Partial<Filter>, merge: boolean) => {
			if (merge)
	...
	return (
		<>
			{!isServerLoaded ? (
				<ServerPicker
					onServerLoaded={() => setIsServerLoaded(true)}
				/>
			) : (
				<>
					<ThemeToggle />
