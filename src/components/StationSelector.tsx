...
const StationSelector: React.FC<StationSelectorProps> = ({
	...
	const [activeFilter, setActiveFilter] = useState<
		"country" | "language" | "tag" | "search" | "favorite" | null
	>(null);
	...
	// Sort options
	const [sortBy, setSortBy] = useState<"name" | "votes" | "clickcount">(
		"votes",
	);
	...
	const fetchStationsByUUIDs = async (
		...
		try {
			...
			if (!stationsPromises.ok) {
				throw new Error(
					`Failed to fetch stations with UUID: ${uuids}`,
				);
			}
		...
		try {
			...
			if (limitedUUIDs.length > 0) {
				...
				const recentStations =
					await fetchStationsByUUIDs(limitedUUIDs);
				const orderedStations = [];
				for (const uuid of limitedUUIDs) {
					const station = recentStations.find(
			...
			console.error(
				"Error loading recently listened stations:",
				err,
			);
			setRecentlyListened([]);
		}
	...
	useEffect(() => {
		const fetchMetadata = async () => {
			try {
				...
				setCountries([
					{ name: "all", stationCount: stationCount },
					...countryOptions,
				]);
				...
				const tagOptions = tagsData
					.filter(
						(item: any) =>
							item.name && item.stationcount > 10,
					)
					.map((item: any) => ({
						name: item.name,
						stationCount: item.stationcount,
	...
	const paginate = (pageNumber: number) => {
		if (
			pageNumber >= 1 &&
			pageNumber <=
				Math.ceil(totalFilteredStations / stationsPerPage)
		) {
			setCurrentPage(pageNumber);
		}
	};
	...
	return (
		<div className="station-selector">
			...
			<div
				className={`filter-group hidden-input-group ${activeFilter === "language" ? "active-filter" : ""}`}
			>
